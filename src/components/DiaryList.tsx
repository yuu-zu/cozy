...
import { Edit3, Trash2, ChevronDown, ChevronUp, Image } from "lucide-react";
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                {/* Đã sửa logic: Giữ nguyên màu sắc và HTML cho cả 2 trạng thái */}
                {isExpanded ? (
                  <div
              ...
              <div className="flex flex-col gap-1">
                <button onClick={() => setExpanded(isExpanded ? null : entry.id)} className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground">
                  {isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </button>
                <button onClick={() => onEdit(entry)} className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={() => onDelete(entry.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive">
